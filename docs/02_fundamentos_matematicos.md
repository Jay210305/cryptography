...
5. **Rejection sampling**: si $\|\mathbf{z}\|_\infty \geq \gamma_1 - \beta$ o si $\|\text{LowBits}(\mathbf{A}\mathbf{z} - c\mathbf{t})\|_\infty \geq \gamma_2 - \beta$, se **aborta y reinicia** desde el paso 1. Esto es crucial: sin rejection sampling, un observador podría extraer información sobre $\mathbf{s}_1$ a partir de múltiples firmas.
...
2. **AAD del cifrado**: $\text{AAD} = \text{SHA3-256}(\text{eph\_pk} \| \text{mlkem\_ct} \| \text{cipher\_id})$ — vincula los parámetros KEM al cifrado AEAD.
